...
with left:
    logo_col, title_col, login_col = st.columns([1, 5, .7])
...
def auth_screen():
    choice = st.radio("Welcome!", ["Signup", "Login"], horizontal=True)
...
if st.session_state.page == "login":
    ...
    with col2:
        st.markdown("<h1 style='font-size:40px; margin-left:8rem;'>Welcome Back! 💝</h1>", unsafe_allow_html=True)
        st.markdown("<p style='font-size:20px; text-align:center;'>Login or Signup</p>", unsafe_allow_html=True)
        auth_screen()
    st.stop()
...
if st.session_state.page == "product":
    ...
    if product["category"] == "Hair":
        option = st.selectbox("Select Length", ["12 inch", "16 inch", "20 inch", "24 inch"])

    if product["category"] == "Nails":
        option = st.selectbox("Select Size", ["Small", "Medium", "Large"])
    ...
    if st.button("Add To Cart"):
        for _ in range(quantity):
            ...
            st.session_state.cart.append({"name": name, "price": product["price"]})
...
page = st.radio("", ["Home", "Shop All", "Hair", "Nails", "Jewelry"], horizontal=True)
...
for i, product in enumerate(products):
    with cols[i % 3]:
        ...
        <a href='?product={i}' target='_self'>
            <img src='data:image/png;base64,{img_base64}' style='width:100%;border-radius:18px;'/>
        </a>
        """, unsafe_allow_html=True)
